...
// ------------------------------------------------------------------------------------
// PILHA PARA DUPLO BALANCEAMENTO (https://programacaodescomplicada.wordpress.com/complementar/)
...
int consulta_topo_Pilha(Pilha* pi, char *conteudo) {
    if (pi == NULL) {
        return 0;
	}
    ...
    conteudo = (*pi)->conteudo;
    return 1;
}

int insere_Pilha(Pilha* pi, char *conteudo) {
    if(pi == NULL) {
        return 0;
	}
